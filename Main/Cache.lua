...
function Cache:PrecacheActor(objname,scale)
    ...
    if obj.AiParams and obj.AiParams.weapon and obj.AiParams.weapon.fireParticle then
...
function Cache:PrecacheItem(objname)
    ...
    if not Cached[hash] then 
        ...
        if obj.DestroyPack then
            ...
            if obj.Destroy.Sound then
                self:PrecacheSounds(obj.Destroy.Sound,"impacts")
            end
            if obj.Destroy.SoundImpact then
                self:PrecacheSounds(obj.Destroy.SoundImpact,"impacts")            
            end
...
function Cache:PrecacheLevel(name)
    ...
    Cached = {}
    PMENU.SetLoadingScreenOverall( PMENU.GetLoadingScreenOverall() + table.getn(GObjects.Elements) + 8, 20 )
    ...
    if Game.GMode ~= GModes.SingleGame then 
        ...
        if Cfg.AllowBrightskins then Game:BrightSkin(entity, true) end
