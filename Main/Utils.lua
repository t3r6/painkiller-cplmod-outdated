...
function SaveFullTable(f,tab,name)
    ...
    local i, v = next(tab, nil)
    while i do
        if not getmetatable(v) and string.sub(i,1,2) ~= "s_" and type(v) ~= "function" then
            local np
            if parent then np = parent[i] end
            if type(v) == "table" then                
                if not IsObjRef(v) then
                    ...
                    if type(i) == "number" then
                        if v.SaveString then 
                            ...
                            SaveFullTable(f,v,name..'['..i..']',diff,np)
                        end
                    else
                        if v.SaveString then 
                            FS.File_Write(f,name..'.'..i..' = '..v:SaveString()..'\n')
                ...
                if type(v) == "boolean" and v == true then val = "true" end                                
                if type(v) == "boolean" and v == false then val = "false" end                                
                if type(v) == "userdata" then val = '"unknown_pointer"' end
                if type(v) == "nil" then val = 'nil' end                
                if type(i) == "number" then
...
function TempObjName()
    ...
    if getfenv()[name] ~= nil then return TempObjName() end
    return name 
end
--============================================================================
function LoadTML(path,file,leveltemplate)
    if not override and Templates[file] then return end -- juz zaladowany (moze jakies dziecko go potrzebowalo?)
    local fname, ext = ParseFileName(file)
    local tml = LoadObj(path.."/"..file,true)
    tml._IsLevelTemplate = leveltemplate
    Templates[file] = tml
    
    --setmetatable(Templates[file],{__mode="k"})
    --MsgBox(file.." = "..tml._Name)
end
--============================================================================
...
function FindObj(name)
    ...
    local o = Templates[name] 
    -- szukam wsrod globalnych obiektow
    if not o then o = getfenv()[name] end
    return o
end
--============================================================================
...
--============================================================================
function BindFX(entity,name,scale,joint,ox,oy,oz,cox,coy,coz)
    local pfx = AddPFX(name,scale)    
    ENTITY.RegisterChild(entity,pfx,true)    
    if type(joint) == "string" then joint = MDL.GetJointIndex(entity, joint) end
    if ox then
        PARTICLE.SetParentOffset(pfx,ox,oy,oz,joint,cox,coy,coz)
    else
        PARTICLE.SetParentOffset(pfx,0,0,0,joint,cox,coy,coz)
    end
    return pfx
end
--============================================================================
