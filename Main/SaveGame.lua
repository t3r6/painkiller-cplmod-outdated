...
SaveGame = 
{
    ...
    LevelTime = "HH:MM:SS",
    CheckPoint = 0,
    Version = 4, -- savegame version (for patches)
}
--============================================================================
...
function SaveGame:RestoreBaseObjectAndClass(obj)
    ...
    if obj.BaseObj then
        local o = FindObj(obj.BaseObj)
        if o then InheritFunctionsAndStatics(obj,o) end
    end
    if obj._Class then
...
--============================================================================
function SaveGame:SaveOtherData(slot)    
    local path = string.format("../SaveGames/%03d/",slot)    
...
--============================================================================
function SaveGame:LoadOtherData(slot)    
    local path = string.format("../SaveGames/%03d/",slot)    
...
--============================================================================
function SaveGame:LoadRequest(slot,loadOnly)
    LoadRequest = 
    {   Slot = slot,
        LoadOnly = loadOnly
    }
...
--============================================================================
function SaveGame:Load(slot,loadOnly)
...
--============================================================================
function SaveGame:SaveRequest(slot,stype)
    SaveRequest = 
    {   Slot = slot,
        SType = stype
    }
...
--============================================================================
function SaveGame:Save(slot,stype,forcetime)
...
function SaveGame:FindLastSave(stype,level)
	...
	for i=1,table.getn(dirs) do
		...
		if o.Type then
			if o.Type == stype and o.Timestamp and SaveGameVersion == o.Version then
