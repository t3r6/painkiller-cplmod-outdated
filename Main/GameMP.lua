...
--============================================================================
-- [SCRIPTS - After Load Level] 
function Game:SetupUpdateClasses()         
    if Game.GMode ~= GModes.MultiplayerClient then return end
    -- setup synchronization classes
    -- classnr, delay, linear vel, angular vel, pos
    NET.SetUpdateClassData(0, Cfg.NetUpdateClass00[1], Cfg.NetUpdateClass00[2], Cfg.NetUpdateClass00[3], Cfg.NetUpdateClass00[4]) -- default (other players and objects)
    NET.SetUpdateClassData(1, Cfg.NetUpdateClass01[1], Cfg.NetUpdateClass01[2], Cfg.NetUpdateClass01[3], Cfg.NetUpdateClass01[4]) -- physics items (barrel, urn..)
    NET.SetUpdateClassData(2, Cfg.NetUpdateClass02[1], Cfg.NetUpdateClass02[2], Cfg.NetUpdateClass02[3], Cfg.NetUpdateClass02[4]) -- rocket, stake, grenade
    NET.SetUpdateClassData(3, Cfg.NetUpdateClass03[1], Cfg.NetUpdateClass03[2], Cfg.NetUpdateClass03[3], Cfg.NetUpdateClass03[4]) -- my player
end
--============================================================================
-- [ENGINE - COMMON] --
function Game:AfterWorldSynchronization(mapName,levelName) 
    ...
    if Game:IsServer() then 
        ...
        MPCfg.FragLimit        = Cfg.FragLimit
        MPCfg.TimeLimit        = Cfg.TimeLimit
        ...
        if MPCfg.GameMode == "Team Deathmatch" or MPCfg.GameMode == "People Can Fly" then
            ...
            Game._countTimer = Cfg.WarmUpTime
        end
...
--    end
    ...
    if Game:IsServer() then 
        Game.SetFragAndTimeLimit(MPCfg.FragLimit,MPCfg.TimeLimit,0)    
    end
...
function Game:AfterNewClientConnected(clientID)
    ...
    SendNetMethod(Game.SetConfiguration,clientID, true, true, Cfg.AllowBrightskins, Cfg.GameMode, Cfg.TeamDamage, MPCfg.GameState)
    ...
    for i,o in self.Players do                
        if o.ClientID ~= clientID then -- bez informacji o sobie, ktora bedzie niezaleznie wyslana do wszystkich klientow
            ...
            if ps then -- ?????? wywalilo kiedys dedicated
                ...
                SendNetMethod(Game.OnNewPlayerInGame, clientID, true, true, o.ClientID, o._Entity, ps.Name, ps.Score, ps.Frags, ps.Deaths, pu_state ,INP.GetTime()-ps._starttime, ps.Team, ps.State, ps.Spectator)
            end
        end
    end    
    -- send limits info & current time
    SendNetMethod(Game.SetFragAndTimeLimit,clientID, true, true,MPCfg.FragLimit,MPCfg.TimeLimit,Game._TimeLimitOut)
    -- reset ping
...
function Game:OnMultiplayerServerTick(delta)
    ...
    if MPCfg.GameState ~= GameStates.Finished then
        ...
        if MPCfg.GameState == GameStates.Counting then
            ...
            if s and self._countTimerInt ~= s then
                if s >= 0 then Game.CountSnd(s) end
                self._countTimerInt = s
            end
            if self._countTimer <= 0 then                                
                Game.SetGameState(GameStates.Playing)                
                if MPCfg.GameMode == "Team Deathmatch" or MPCfg.GameMode == "People Can Fly" then -- autorespawn                    
            ...
            self._countTimer = self._countTimer - delta 
        end       
    end
    ...
    if pc > 0 then
        ...
        if self.YawTick > 1/15 then
            for i,o in self.Players do        
                local j = 1
                local marg = {}
                for i,o in self.Players do
                    marg[j] = o.ClientID
                    marg[j+1] = (o._yaw + math.pi/2) * (65535/math.pi)
                    marg[j+2] = o.State
                    --Game:Print("send:"..o.State)
                    j = j + 3
                end
                self.PlayerCamYaw(table.getn(self.Players),unpack(marg))
            end
            self.YawTick = 0
        end
...
--============================================================================
function Game:CountSnd(sec)
    SOUND.Play2D("multiplayer/clock-tick",100,true,true)
    CONSOLE.AddMessage(sec)           
end
Network:RegisterMethod("Game.CountSnd", NCallOn.AllClients, NMode.Unreliable, "b")
--============================================================================
-- [ENGINE - CLIENT ONLY]  
    ...
    if not self._procStats then
		if MPCfg.GameState == GameStates.WarmUp then
			if INP.UIAction(UIActions.Scoreboard) then
				local t = nil
				if MPCfg.GameMode == "Team Deathmatch" or MPCfg.GameMode == "People Can Fly" then t = 2  end
		...
		if INP.UIAction(UIActions.Scoreboard) and
		   (MPCfg.GameState == GameStates.WarmUp or (MPCfg.GameState == GameStates.Counting and (MPCfg.GameMode == "Team Deathmatch" or MPCfg.GameMode == "People Can Fly"))) then
...
function Game:OnNewPlayerInGame(clientID,entity,name,score,frags,deaths,pu_state,playtime,team,state,spectator,t1score,t2score)
    ...
    Game.PlayerStats[clientID] = {ClientID = clientID, Name = name, Score = score, Frags = frags, Deaths = deaths, Ping = 0, PacketLoss = 0, Team = team, State = state, Spectator = spectator}
    local ps = Game.PlayerStats[clientID]
    MPSTATS.Update(ps.ClientID,ps.Name, ps.Score, ps.Frags, ps.Deaths, ps.Ping, ps.PacketLoss, ps.Team, ps.State, ps.Spectator)
    ps._Entity = entity    
    ps.Player = player    
    ps._respawned = false
    ...
    if spectator == 0 then
        ...
        if Game.GMode ~= GModes.DedicatedServer then
            local p = GObjects:Add(TempObjName(),PPlayerAnimation:New(nil,clientID))         
...
--============================================================================
-- [NET - SERVER & ALL CLIENTS]  
function Game:SetFragAndTimeLimit(fraglimit,timelimit,timeout)
...
Network:RegisterMethod("Game.SetFragAndTimeLimit", NCallOn.ServerAndAllClients, NMode.Reliable, "fff")
--============================================================================
-- [NET - SERVER & ALL CLIENTS]  
...
function Game:PlayerRespawnRequest(clientID)
    ...
    if player and not player._Entity then    
        ...
        local exist,x,y,z,a,pt = FindFreeRespawnPoint(player._lastRespawnPoint)
...
function Game:NewPlayerTeamConfirmation(clientID,team)
    ...
    if ps then
        ...
        --else        
            CONSOLE.AddMessage(ps.Name.." is now in team: "..(team+1)..".")
            ps.Team  = team
...
--============================================================================
function Game:CheckTDMStatus()        
    
    if MPCfg.GameMode ~= "Team Deathmatch" and MPCfg.GameMode ~= "People Can Fly" then return end
    ...
    local pbreak = 0
    for i,o in Game.PlayerStats do
        if o.Spectator == 0 and o.State ~= 1 then
    ...
    if Game:IsServer() then 
        if MPCfg.GameState == GameStates.Playing and pbreak > n/2 then            
...
function Game:StartVotingRequest(clientID,cmd,params)
	...
	if cmd == "kick" and Cfg.UserKick then
		allowed = true
	elseif cmd == "bankick" and Cfg.UserBankick then
		allowed = true
...
--============================================================================
-- [NET - ALL CLIENTS]  
function Game:PlayerCamYaw(cnt,...)        
    local i = 1
    while arg[i] do 
        local ps = Game.PlayerStats[arg[i]]
        if ps and ps._animproc then  
            ps._animproc.Yaw =  arg[i+1] / (65535/math.pi) - math.pi/2            
            ps._animproc.State = arg[i+2]
            --Game:Print("doszlo:"..ps._animproc.State)
            --Game:Print("doszlo:"..ps._animproc._Name)
        end
        NET.RecordPlayersInfo( arg[i],arg[i+1] )
        i = i + 3
    end
end
Network:RegisterMethod("Game.PlayerCamYaw", NCallOn.AllClients, NMode.Unreliable, "xbub")
--============================================================================
-- [NET - SERVER] --
...
--============================================================================
-- [ NET - ALL CLIENTS ]  
function Game:SetConfiguration(brightskins,gamemode,teamdamage)
    MPCfg.AllowBrightskins = brightskins
    MPCfg.TeamDamage = teamdamage
    MPCfg.GameMode = gamemode
    CONSOLE.AddMessage(gamemode)
end
Network:RegisterMethod("Game.SetConfiguration", NCallOn.AllClients, NMode.Reliable, "BsB")
--============================================================================
-- [ NET - SERVER & ALL CLIENTS ]
