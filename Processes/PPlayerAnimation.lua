...
PPlayerAnimation = {
    ...
    _LastYaw = 0,
    _LastJumpTimer = 0,
    _LastPos   = Vector:New(0,0,0),
    _Entity = nil,
    _mdlPKHead = nil,
    _mshMuzzle = nil,
    TipPoint = Vector:New(0,0,0),
    _tmElectro = 0,
    _wallFryTimeOut =0,
    _Class  = "CProcess",        
    s_SubClass = 
...
function PPlayerAnimation:New(entity,clientid)
    local p = Clone(PPlayerAnimation)
    p.BaseObj = "PPlayerAnimation"
        ...
        --if not (Player and Player._Entity == e) then 
            ...
            if (anim == "jump_land" or anim == "jump_f_land" or anim == "jump_b_land") then
                ...
                if vy < -11 then
                    ...
                    AddPFX("but", 0.15 ,Vector:New(x+dx*2,y-0.3,z+dz*2))
                end
            end
        --end
...
function PPlayerAnimation:ShowCorrectWeapon()
    ...
    if self.State == 51 then
        ...
        local q = Quaternion:New_FromEuler(self.Yaw,ENTITY.GetOrientation(e),0)
...
function PPlayerAnimation:FireFX(delta)    
    ...
    if self._waitFX <= 0 then
        if self.State == 41 then
            ...
            if Player and self.ClientID == Player.ClientID then -- my player
                fx,fy,fz = Player.ForwardVector.X, Player.ForwardVector.Y, Player.ForwardVector.Z
                b,d,x,y,z,nx,ny,nz,he,e = Player:Trace(s.AltFireRange)
            else -- other
                local q = Quaternion:New_FromEuler(self.Yaw,ENTITY.GetOrientation(self._Entity),0)
                fx,fy,fz = q:InverseTransformVector(0,0,1)                                
                local cx,cy,cz = ENTITY.PO_GetPawnHeadPos(self._Entity)
                ENTITY.RemoveFromIntersectionSolver(self._Entity)
                b,d,x,y,z,nx,ny,nz,he,e = WORLD.LineTrace(cx,cy,cz,cx+fx*s.AltFireRange,cy+fy*s.AltFireRange,cz+fz*s.AltFireRange)
...
function PPlayerAnimation:Tick(delta)
    ...
    if not MDL.IsRagdollActive(e) then        
        ...
        ENTITY.RemoveFromIntersectionSolver(self._Entity)
        local isFloor = WORLD.LineTrace(x,y+0.5,z,x,y-0.65,z)    
        ENTITY.AddToIntersectionSolver(self._Entity)
        ...
        local yaw = self._LastYaw + (self.Yaw - self._LastYaw) * 0.33 
        self._test = self._test + delta
        if self._test > 3.14 then self._test = -3.14 end
        local joint = MDL.GetJointIndex(e, "K_ogo")
        if yaw > 0 then
            --MDL.SetHeadTrackRot(e, yaw/2, yaw, 0)
            MDL.ApplyJointRotation(e, joint, yaw/2, yaw, 0)
        else
            --MDL.SetHeadTrackRot(e, -yaw/2, yaw, 0)
            MDL.ApplyJointRotation(e, joint, -yaw/2, yaw, 0)
        end
    ...
    if not (Player and Player._Entity == entity) or Game.TPP then    
        ...
        if self.State == 12 then -- painkiller ray
            ...
            if pe ~= 0 then
                ...
                local q = Quaternion:New_FromEuler(self.Yaw,ENTITY.GetOrientation(entity),0)
                local fx,fy,fz = q:InverseTransformVector(0,0,1)                    
