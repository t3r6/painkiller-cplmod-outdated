...
--	bgType		= MenuBackgroundTypes.Movie,
	...
	{
		{
			...
			DynamicLights = false,			-- true/false
	...
	ConnectionSpeedPresets =
	{
		...
		{
			-- LAN/T1
			NetUpdateClass00 = { 0.01, 0.01, 0.01, 0.01 }, -- default (other players and objects)
			NetUpdateClass01 = { 0.01, 0.01, 0.01, 0.01 }, -- physics items (barrel, urn..)
			NetUpdateClass02 = { 0.01, 0.01, 0.01, 0.01 }, -- rocket, stake, grenade
			NetUpdateClass03 = { 0.01, 0.01, 0.01, 0.01 }, -- my player
			NetUpdateMaxPacketSize = 300,
			NetUpdateSendPeriod = 10, -- miliseconds
		},
...
--	PainMenu:OnMovieLoopEnd()
	...
	elseif self.currScreen == NewGameMenu then
		...
		for i=1,table.getn(Game.CardsAvailable) do
			if Game.CardsAvailable[i] then cardCount = cardCount + 1 end
		end
		if cardCount > 22 then
...
--				Game:Print( "Adding loadsave" )
		...
		elseif o.type == MenuItemTypes.TextButtonEx then
			...
			for i, v in o.values do
				if itemName ~= "ModelSelect" or i < table.getn(o.values) or Game.BonusMPModel or IsMPDemo() then
...
function PainMenu:SetupItem( screen, i, o )
	...
	if screen == VideoOptions and R3D.GetHWClass() == R3DHWClass.GenericTnL then
		...
		screen.items.AdvancedTab.items.WeaponSpecular.disabled = 1
		screen.items.AdvancedTab.items.WeaponNormalMap.disabled = 1
...
--	PMENU.AddSaveGameToList( name, "header", TXT.Menu.Level, TXT.SPStats.Playtime, TXT.Menu.SaveTime, TXT.SPStats.Difficulty )
	...
	for i=1,table.getn(dirs) do
		...
		if o.Type and SaveGameVersion == o.Version then
...
function PainMenu:SwapTextButtonEx( name, inc )
	...
	if inc == 1 then
		...
		if name == "ModelSelect" and item.currValue == table.getn(item.values) and not (Game.BonusMPModel or IsMPDemo()) then
			item.currValue = 1
		end
	else
...
--	if name == "CDKey" then
--		PMENU.SaveCDKey( PMENU.GetTextEditValue( name ) )
--	else
...
--	WORLD.Release(true)
	...
	if not mode or mode == 1 then
		if Game.GameInProgress then
			PainMenu:AskYesNo( Languages.Texts[686], 'Game.GameInProgress = false; Game.LevelStarted = false; Game:ClearLevelsStats(false); PainMenu:SignAPact(2)', 'PainMenu:ActivateScreen(MainMenu)' )
		else
			PainMenu:SignAPact(2)
		end
	else
		for i=1,table.getn(Game.CardsAvailable) do
		...
		if ask then
			PainMenu:AskYesNo( Languages.Texts[636], 'Game.GameInProgress = false; Game.LevelStarted = false; if Game then Game:ClearLevelsStats(true) end; PainMenu:ActivateScreen(NewGameMenu)', 'Game.GameInProgress = false; Game.LevelStarted = false; PainMenu:ActivateScreen(NewGameMenu)' )
		else
			Game.GameInProgress = false
			Game.LevelStarted = false
			PainMenu:ActivateScreen(NewGameMenu)
