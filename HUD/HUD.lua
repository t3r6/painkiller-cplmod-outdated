...
--==CPLHUD
	if Cfg.CPLHud then
        Hud._matTeamRed         = MATERIAL.Create("../CPLGui/Textures/team_red.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matTeamBlue        = MATERIAL.Create("../CPLGui/Textures/team_blue.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matShell           = MATERIAL.Create("../CPLGui/Textures/Shell.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matFreezer         = MATERIAL.Create("../CPLGui/Textures/Freezer.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matRocket          = MATERIAL.Create("../CPLGui/Textures/Rocket.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matMinigun         = MATERIAL.Create("../CPLGui/Textures/Minigun.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matStake           = MATERIAL.Create("../CPLGui/Textures/Stake.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matSzuriken        = MATERIAL.Create("../CPLGui/Textures/Szuriken.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matElectro         = MATERIAL.Create("../CPLGui/Textures/Electro.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matPainkiller      = MATERIAL.Create("../CPLGui/Textures/Painkiller.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
        Hud._matInfinity        = MATERIAL.Create("../CPLGui/Textures/infinity.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
	Hud._matout        	= MATERIAL.Create("../CPLGui/Textures/out.tga", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
end 
...
if Player then
		    ...
		    if Cfg.HUD_AmmoList_Show then
        		Hud:AmmoList()
    				end

		    if MPCfg.GameMode == "Team Deathmatch"  then
        		Hud:DrawTeamScores()
    				end

    		   if Cfg.HUD_CurrentWeapon_Icon then
        		Hud:CurrentWeaponIcon()
    				end
	...
	if Game and Game.GMode ~= GModes.SingleGame and (self._showtimer or Cfg.ShowTimer) and Game._TimeLimitOut then
		...
		HUD.SetFont("timesbd",42)
		if (self._showtimer or Cfg.ShowTimer) and Game.GMode ~= GModes.SingleGame then
			HUD.PrintXY((Cfg.HUD_Timer_Minutes_Pos_X*w/1024),Cfg.HUD_Timer_Minutes_Pos_Y*h/768,time,"timesbd",15,15,15,42)
			if red then
				HUD.PrintXY((Cfg.HUD_Timer_Minutes_Pos_X*w/1024),Cfg.HUD_Timer_Minutes_Pos_Y*h/768,time,"timesbd",230,0,0,42)
			else
				HUD.PrintXY((Cfg.HUD_Timer_Minutes_Pos_X*w/1024),Cfg.HUD_Timer_Minutes_Pos_Y*h/768,time,"timesbd",255,255,255,42)
			end 
		else if Game.GMode ~= GModes.SingleGame then
			HUD.PrintXY(w-HUD.GetTextWidth(time)+1,1,time,"timesbd",15,15,15,36)
			if red then
				HUD.PrintXY((Cfg.HUD_Timer_Minutes_Pos_X*w/1024),Cfg.HUD_Timer_Minutes_Pos_Y*h/768,0,time,"timesbd",230,0,0,42)
			else
				HUD.PrintXY((Cfg.HUD_Timer_Minutes_Pos_X*w/1024),Cfg.HUD_Timer_Minutes_Pos_Y*h/768,0,time,"timesbd",255,255,255,42)
			end 
			end
	  	end 
	 end
	 ...
	 --SHOWFPS
	if Cfg.HUD_FPS_Show then
        local fps = string.format("%d",R3D.GetFPS())
	HUD.SetFont("timesbd",26)
        HUD.PrintXY(((Cfg.HUD_FPS_Pos_X-Cfg.HUD_FPS_Text_Interval)*w/1024)+1,(Cfg.HUD_FPS_Pos_Y*h/768)+1,"fps","timesbd",15,15,15,26)
        HUD.PrintXY((Cfg.HUD_FPS_Pos_X-Cfg.HUD_FPS_Text_Interval)*w/1024,Cfg.HUD_FPS_Pos_Y*h/768,"fps","timesbd",200,200,170,26)

        HUD.PrintXY((Cfg.HUD_FPS_Pos_X*w/1024)+1,(Cfg.HUD_FPS_Pos_Y*h/768)+1,""..fps.."","timesbd",200,200,170,26)
        HUD.PrintXY(Cfg.HUD_FPS_Pos_X*w/1024,Cfg.HUD_FPS_Pos_Y*h/768,""..fps.."","timesbd",200,200,170,26)
        end
...
--			Game:Print( "Update" )
		...
		if Game.MegaBossHealth and Game.MegaBossHealthMax then
			...
			if size == 0 and Game.MegaBossHealth > 0 then size = 1 end
...
function Hud:AmmoList()
    ...
    if Cfg.HUD_AmmoList_Pos == 1 then
        Hud:AmmoListPos1()
    elseif Cfg.HUD_AmmoList_Pos == 2 then
        Hud:AmmoListPos2()
    end

    Hud:QuadTrans(Hud._matShell,AmmoList_Icons_Pos1_X,AmmoList_Pos1_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,Cfg.HUD_AmmoList_Icons_Transp)
    Hud:QuadTrans(Hud._matFreezer,AmmoList_Icons_Pos2_X,AmmoList_Pos2_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,Cfg.HUD_AmmoList_Icons_Transp)
    Hud:QuadTrans(Hud._matStake,AmmoList_Icons_Pos3_X,AmmoList_Pos3_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,Cfg.HUD_AmmoList_Icons_Transp)
    Hud:QuadTrans(Hud._matRocket,AmmoList_Icons_Pos4_X,AmmoList_Pos4_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,Cfg.HUD_AmmoList_Icons_Transp)
    Hud:QuadTrans(Hud._matMinigun,AmmoList_Icons_Pos5_X,AmmoList_Pos5_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,Cfg.HUD_AmmoList_Icons_Transp)
    Hud:QuadTrans(Hud._matSzuriken,AmmoList_Icons_Pos6_X,AmmoList_Pos6_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,Cfg.HUD_AmmoList_Icons_Transp)
    Hud:QuadTrans(Hud._matElectro,AmmoList_Icons_Pos7_X,AmmoList_Pos7_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,Cfg.HUD_AmmoList_Icons_Transp)

    if Cfg.HUD_AmmoList_Erasures then
        Hud:Erasures()
    end

    if MPCfg.GameMode == "Team Deathmatch" and Player.Ammo.Shotgun == 999 then
        Hud:Quad(Hud._matInfinity,AmmoList_Text_Pos1_X,AmmoList_Pos1_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
        Hud:Quad(Hud._matInfinity,AmmoList_Text_Pos2_X,AmmoList_Pos2_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
        Hud:Quad(Hud._matInfinity,AmmoList_Text_Pos3_X,AmmoList_Pos3_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
        Hud:Quad(Hud._matInfinity,AmmoList_Text_Pos4_X,AmmoList_Pos4_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
        Hud:Quad(Hud._matInfinity,AmmoList_Text_Pos5_X,AmmoList_Pos5_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
        Hud:Quad(Hud._matInfinity,AmmoList_Text_Pos6_X,AmmoList_Pos6_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
        Hud:Quad(Hud._matInfinity,AmmoList_Text_Pos7_X,AmmoList_Pos7_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
    elseif MPCfg.GameMode == "People Can Fly" then
        Hud:Quad(self._matInfinity,AmmoList_Text_Pos4_X,AmmoList_Pos4_Y+(-1*h/768),Cfg.HUD_AmmoList_Scale*1.2,false)
        Hud:DrawDigitsText(AmmoList_Text_Pos5_X+(10*w/1024),AmmoList_Pos5_Y+(4*h/768),string.format("%3d",Player.Ammo.MiniGun),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.MiniGun,255)
    else
        Hud:DrawDigitsText(AmmoList_Text_Pos1_X+(10*w/1024),AmmoList_Pos1_Y+(4*h/768),string.format("%3d",Player.Ammo.Shotgun),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.Shotgun,Cfg.HUD_AmmoList_Digits_Transp,Cfg.HUD_AmmoList_Digits_Color_R,Cfg.HUD_AmmoList_Digits_Color_G,Cfg.HUD_AmmoList_Digits_Color_B,Cfg.HUD_AmmoList_Digits_ColorRed_R,Cfg.HUD_AmmoList_Digits_ColorRed_G,Cfg.HUD_AmmoList_Digits_ColorRed_B)
        Hud:DrawDigitsText(AmmoList_Text_Pos2_X+(10*w/1024),AmmoList_Pos2_Y+(4*h/768),string.format("%3d",Player.Ammo.IceBullets),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.IceBullets,Cfg.HUD_AmmoList_Digits_Transp,Cfg.HUD_AmmoList_Digits_Color_R,Cfg.HUD_AmmoList_Digits_Color_G,Cfg.HUD_AmmoList_Digits_Color_B,Cfg.HUD_AmmoList_Digits_ColorRed_R,Cfg.HUD_AmmoList_Digits_ColorRed_G,Cfg.HUD_AmmoList_Digits_ColorRed_B)
        Hud:DrawDigitsText(AmmoList_Text_Pos3_X+(10*w/1024),AmmoList_Pos3_Y+(4*h/768),string.format("%3d",Player.Ammo.Stakes),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.Stakes,Cfg.HUD_AmmoList_Digits_Transp,Cfg.HUD_AmmoList_Digits_Color_R,Cfg.HUD_AmmoList_Digits_Color_G,Cfg.HUD_AmmoList_Digits_Color_B,Cfg.HUD_AmmoList_Digits_ColorRed_R,Cfg.HUD_AmmoList_Digits_ColorRed_G,Cfg.HUD_AmmoList_Digits_ColorRed_B)
        Hud:DrawDigitsText(AmmoList_Text_Pos4_X+(10*w/1024),AmmoList_Pos4_Y+(4*h/768),string.format("%3d",Player.Ammo.Grenades),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.Grenades,Cfg.HUD_AmmoList_Digits_Transp,Cfg.HUD_AmmoList_Digits_Color_R,Cfg.HUD_AmmoList_Digits_Color_G,Cfg.HUD_AmmoList_Digits_Color_B,Cfg.HUD_AmmoList_Digits_ColorRed_R,Cfg.HUD_AmmoList_Digits_ColorRed_G,Cfg.HUD_AmmoList_Digits_ColorRed_B)
        Hud:DrawDigitsText(AmmoList_Text_Pos5_X+(10*w/1024),AmmoList_Pos5_Y+(4*h/768),string.format("%3d",Player.Ammo.MiniGun),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.MiniGun,Cfg.HUD_AmmoList_Digits_Transp,Cfg.HUD_AmmoList_Digits_Color_R,Cfg.HUD_AmmoList_Digits_Color_G,Cfg.HUD_AmmoList_Digits_Color_B,Cfg.HUD_AmmoList_Digits_ColorRed_R,Cfg.HUD_AmmoList_Digits_ColorRed_G,Cfg.HUD_AmmoList_Digits_ColorRed_B)
        Hud:DrawDigitsText(AmmoList_Text_Pos6_X+(10*w/1024),AmmoList_Pos6_Y+(4*h/768),string.format("%3d",Player.Ammo.Shurikens),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.Shurikens,Cfg.HUD_AmmoList_Digits_Transp,Cfg.HUD_AmmoList_Digits_Color_R,Cfg.HUD_AmmoList_Digits_Color_G,Cfg.HUD_AmmoList_Digits_Color_B,Cfg.HUD_AmmoList_Digits_ColorRed_R,Cfg.HUD_AmmoList_Digits_ColorRed_G,Cfg.HUD_AmmoList_Digits_ColorRed_B)
        Hud:DrawDigitsText(AmmoList_Text_Pos7_X+(10*w/1024),AmmoList_Pos7_Y+(4*h/768),string.format("%3d",Player.Ammo.Electro),0.8*Cfg.HUD_AmmoList_Scale,Player.s_SubClass.AmmoWarning.Electro,Cfg.HUD_AmmoList_Digits_Transp,Cfg.HUD_AmmoList_Digits_Color_R,Cfg.HUD_AmmoList_Digits_Color_G,Cfg.HUD_AmmoList_Digits_Color_B,Cfg.HUD_AmmoList_Digits_ColorRed_R,Cfg.HUD_AmmoList_Digits_ColorRed_G,Cfg.HUD_AmmoList_Digits_ColorRed_B)
    end
end
--=====================================================================================
function Hud:AmmoListPos1()
    ...
    local icons_pos_x = (5+Cfg.HUD_AmmoList_Pos_X)*w/1024
    ...
    local text_pos_x = (((45)*Cfg.HUD_AmmoList_Scale)+Cfg.HUD_AmmoList_Pos_X)*w/1024
    ...
    AmmoList_Pos1_Y             = (Cfg.HUD_AmmoList_Pos_Y+((0)*Cfg.HUD_AmmoList_Scale))*h/768
    AmmoList_Pos2_Y             = (Cfg.HUD_AmmoList_Pos_Y+((37)*Cfg.HUD_AmmoList_Scale))*h/768
    AmmoList_Pos3_Y             = (Cfg.HUD_AmmoList_Pos_Y+((74)*Cfg.HUD_AmmoList_Scale))*h/768
    AmmoList_Pos4_Y             = (Cfg.HUD_AmmoList_Pos_Y+((111)*Cfg.HUD_AmmoList_Scale))*h/768
    AmmoList_Pos5_Y             = (Cfg.HUD_AmmoList_Pos_Y+((148)*Cfg.HUD_AmmoList_Scale))*h/768
    AmmoList_Pos6_Y             = (Cfg.HUD_AmmoList_Pos_Y+((185)*Cfg.HUD_AmmoList_Scale))*h/768
    AmmoList_Pos7_Y             = (Cfg.HUD_AmmoList_Pos_Y+((222)*Cfg.HUD_AmmoList_Scale))*h/768
end
--=====================================================================================
function Hud:AmmoListPos2()
    local w,h = R3D.ScreenSize()

    local icons_pos_x = w-(((75*Cfg.HUD_AmmoList_Scale)-32+Cfg.HUD_AmmoList_Pos_X)*w/1024)
    ...
    local text_pos_x = w-(((165*Cfg.HUD_AmmoList_Scale)-50+Cfg.HUD_AmmoList_Pos_X)*w/1024)
...
--=====================================================================================
function Hud:DrawTeamScores()
    local w,h = R3D.ScreenSize()

    scorelistfont = "courbd"

    if Cfg.HUD_TeamScores_Icons_Show then
        Hud:QuadTrans(Hud._matTeamRed,Cfg.HUD_TeamScores_Team1_Icon_Pos_X*w/1024,Cfg.HUD_TeamScores_Team1_Icon_Pos_Y*h/768,Cfg.HUD_TeamScores_Team1_Icon_Scale,false,255)
        Hud:QuadTrans(Hud._matTeamBlue,Cfg.HUD_TeamScores_Team2_Icon_Pos_X*w/1024,Cfg.HUD_TeamScores_Team2_Icon_Pos_Y*h/768,Cfg.HUD_TeamScores_Team2_Icon_Scale,false,255)
    end

    if Cfg.HUD_TeamScores_Names_Show then
        HUD_TeamScores_Team1_Name = Cfg.HUD_TeamScores_Team1_Name
        HUD_TeamScores_Team2_Name = Cfg.HUD_TeamScores_Team2_Name
    elseif not Cfg.HUD_TeamScores_Names_Show then
        HUD_TeamScores_Team1_Name = ""
        HUD_TeamScores_Team2_Name = ""
    end

    if Cfg.HUD_TeamScores_Box_Show then
        HUD.DrawBorder(Cfg.HUD_TeamScores_Box_Pos_X,Cfg.HUD_TeamScores_Box_Pos_Y,Cfg.HUD_TeamScores_Box_Size_X,Cfg.HUD_TeamScores_Box_Size_Y)
    end

    if Cfg.HUD_TeamScores_Show then
        HUD.PrintXY((Cfg.HUD_TeamScores_Team1_Pos_X*w/1024)+1,(Cfg.HUD_TeamScores_Team1_Pos_Y*h/768)+1,""..HUD_TeamScores_Team1_Name..""..string.format("%3d",Game._team1Score).."",scorelistfont,0,0,0,Cfg.HUD_TeamScores_Team1_Font_Size)
        HUD.PrintXY((Cfg.HUD_TeamScores_Team1_Pos_X*w/1024),Cfg.HUD_TeamScores_Team1_Pos_Y*h/768,""..HUD_TeamScores_Team1_Name..""..string.format("%3d",Game._team1Score).."",scorelistfont,Cfg.HUD_TeamScores_Team1_Color_R,Cfg.HUD_TeamScores_Team1_Color_G,Cfg.HUD_TeamScores_Team1_Color_B,Cfg.HUD_TeamScores_Team1_Font_Size)

        HUD.PrintXY((Cfg.HUD_TeamScores_Team2_Pos_X*w/1024)+1,(Cfg.HUD_TeamScores_Team2_Pos_Y*h/768)+1,""..HUD_TeamScores_Team2_Name..""..string.format("%3d",Game._team2Score).."",scorelistfont,0,0,0,Cfg.HUD_TeamScores_Team2_Font_Size)
        HUD.PrintXY((Cfg.HUD_TeamScores_Team2_Pos_X*w/1024),Cfg.HUD_TeamScores_Team2_Pos_Y*h/768,""..HUD_TeamScores_Team2_Name..""..string.format("%3d",Game._team2Score).."",scorelistfont,Cfg.HUD_TeamScores_Team2_Color_R,Cfg.HUD_TeamScores_Team2_Color_G,Cfg.HUD_TeamScores_Team2_Color_B,Cfg.HUD_TeamScores_Team2_Font_Size)
    end
end
--=====================================================================================
function Hud:CurrentWeaponIcon()
    ...
    if Player._CurWeaponIndex == 1 then
        Hud:QuadTrans(Hud._matPainkiller,(15+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_X)*w/1024,(50+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_Y)*h/768,1,false,255)
    elseif Player._CurWeaponIndex == 2 then
        Hud:QuadTrans(Hud._matShell,(15+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_X)*w/1024,(50+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_Y)*h/768,1,false,255)
    elseif Player._CurWeaponIndex == 3 then
        Hud:QuadTrans(Hud._matStake,(15+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_X)*w/1024,(50+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_Y)*h/768,1,false,255)
    elseif Player._CurWeaponIndex == 4 then
        Hud:QuadTrans(Hud._matRocket,(15+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_X)*w/1024,(50+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_Y)*h/768,1,false,255)
    elseif Player._CurWeaponIndex == 5 then
        Hud:QuadTrans(Hud._matElectro,(15+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_X)*w/1024,(50+Cfg.HUD_CurrentWeapon_Icon_Pos_Modif_Y)*h/768,1,false,255)
    end
end
--=====================================================================================
function Hud:Erasures()
    if Player.EnabledWeapons[2] == Shotgun then
        Hud:QuadTrans(Hud._matout,AmmoList_Icons_Pos1_X,AmmoList_Pos1_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,255)
        Hud:QuadTrans(Hud._matout,AmmoList_Icons_Pos2_X,AmmoList_Pos2_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,255)
    end

    if Player.EnabledWeapons[3] == StakeGunGL then
        Hud:QuadTrans(Hud._matout,AmmoList_Icons_Pos3_X,AmmoList_Pos3_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,255)
    end

    if Player.EnabledWeapons[3] == StakeGunGL or Player.EnabledWeapons[4] == MiniGunRL then
        Hud:QuadTrans(Hud._matout,AmmoList_Icons_Pos4_X,AmmoList_Pos4_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,255)
    end

    if Player.EnabledWeapons[4] == MiniGunRL then
        Hud:QuadTrans(Hud._matout,AmmoList_Icons_Pos5_X,AmmoList_Pos5_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,255)
    end

    if Player.EnabledWeapons[5] == DriverElectro then
        Hud:QuadTrans(Hud._matout,AmmoList_Icons_Pos6_X,AmmoList_Pos6_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,255)
        Hud:QuadTrans(Hud._matout,AmmoList_Icons_Pos7_X,AmmoList_Pos7_Y,Cfg.HUD_AmmoList_Scale*Cfg.HUD_AmmoList_Icons_Scale,false,255)
    end
--=====================================================================================
end