...
CPlayer = 
{
    ...
    EnabledWeapons = {"PainKiller","Shotgun","StakeGunGL","MiniGunRL","DriverElectro"},
...
function CPlayer:ClientTick(delta)
    ...
    if self._died or not self._Entity then 
        if Game.GMode == GModes.MultiplayerClient  then -- pusty pakiet pingujacy
            NET.SendEmptyPacket()
        end
        return 
    end
    ...
    -- fire best weapon 1
    if INP.Action(Actions.FireBestWeapon1) or INP.Action(Actions.FireBestWeapon2) then
    ...
    if self:GetCurWeapon() then        
        if (not IsFinalBuild() and INP.Key(Keys.G)==2) or Game.TPP or Cfg.ViewWeaponModel == false then
    ...
    RawCallMethod(CPlayer.SetupAction,self.ClientID,action,ax,ay)        
    ...
    if Game.GMode == GModes.MultiplayerClient  then
        ...
        if not IsConstPhysicsTick() then
            if self.LastAction ~= self.CurAction or NET.ActionSend_Test() then
                CPlayer.SetupAction(self.ClientID,self.CurAction,ax,ay)
        ...
        PLAYER.AddLocalAction( self.CurAction, ax, ay )
        PLAYER.ExecMultiPlayerAction(self._Entity,0,fv.X,fv.Y,fv.Z,rv.X,rv.Y,rv.Z)
    end

    if self:GetCurWeapon() then   self:GetCurWeapon():ClientTick(delta) end        
end
...
function CPlayer:ServerTick(delta)
    ...
    if self._waitingForFirstFire > 0 then self._waitingForFirstFire = self._waitingForFirstFire - delta end
    ...
    if self._waitingForFirstFire <= 0 then
...
function CPlayer:TryToChangeWeapon(slot)    
    ...
    if self.EnabledWeapons[slot] and slot ~= self._CurWeaponIndex then
        ...
        if not cw or cw.ShotTimeOut < 0 then --and not ENTITY.PO_IsActionState(self._Entity,Actions.Fire) and not ENTITY.PO_IsActionState(self._Entity,Actions.AltFire) then
            ...
            if not self._SwitchToWeapon and not ENTITY.PO_IsActionState(self._Entity,Actions.FireBestWeapon1) and not ENTITY.PO_IsActionState(self._Entity,Actions.FireBestWeapon2) then
...
function CPlayer:InterpretAction(delta)
    ...
    elseif ENTITY.PO_IsActionState(self._Entity,Actions.ForwardRocketJump) then
        self:TryToChangeWeapon(4)
    elseif ENTITY.PO_IsActionState(self._Entity,Actions.RocketJump) then
        self:TryToChangeWeapon(4)
    elseif  self._SwitchToWeapon then -- queued weapon switch
        self:TryToChangeWeapon(self._SwitchToWeapon)
    end
    
    --Game:Print("--------------------")
    --Game:Print(GetCallStackInfo(1))
    --Game:Print(GetCallStackInfo(2))
    --Game:Print(GetCallStackInfo(3))
    --Game:Print(GetCallStackInfo(4))
    --FireBestExplosive	= 2*256*256*256,
    --FireBestNonExplosive = 4*256*256*256,
    --FireBestWeapon		= 8*256*256*256,
...
function CPlayer:Tick(delta)
    ...
    if Game.GMode == GModes.SingleGame then     
        ...
        if not self._jumped and jumped then
...
function CPlayer:ClientTick2(delta)
    ...
    if Game._EarthQuakeProc and Game._EarthQuakeProc._noEquake then
...
function CPlayer:PostRender(delta)
	if self.TickCount then
		...
		self.TickCount = self.TickCount + (delta * 2 * math.pi / --[[self.Time--]](2*math.pi/30))
		Color.A = --[[self.MaxAlpha--]]60 * (1-(1+math.cos(self.TickCount))/2)
		local w,h = R3D.ScreenSize()
...
function CPlayer:OnDamage(damage,killer,attack_type,x,y,z,nx,ny,nz)
    ...
    if MPCfg.GameState ~= GameStates.Playing then        
        if attack_type == AttackTypes.OutOfLevel and MPCfg.GameState ~= GameStates.Finished then
            local exist,x,y,z,a,pt = FindFreeRespawnPoint(nil,true)
	...
	if ( GOD and not (IsFinalBuild() and attack_type==AttackTypes.OutOfLevel) ) or (Game.IsDemon and not Lucifer_001 and attack_type ~= AttackTypes.OutOfLevel) or Game.PlayerNoDamage == true then 
        self._frags = nil
        return         
    end
    ...
    if Game.GMode == GModes.SingleGame then
        ...
        if attack_type ~= AttackTypes.OutOfLevel and attack_type ~= AttackTypes.TeleFrag then
            ...
            if MPCfg.GameMode == "Team Deathmatch" and not MPCfg.TeamDamage and killer and MPCfg.GameState == GameStates.Playing then
    ...
    if not self._died then
        ...
        if self.Health > 0 then
            ...
            CPlayer:Common_UpdateStats(true,self.ClientID,kID,attack_type,gib,frags)
            ...
            CPlayer.Client_OnDeath(self.ClientID,kID,attack_type,gib,frags,damage)                        
        ...
        if Game.GMode == GModes.SingleGame and nx then
...
--============================================================================
function FindFreeRespawnPoint(last,always)
...
function CPlayer:ResetStatus(weapon)    
    ...
    if MPCfg.GameMode == "Voosh" then        
        ...
        self.EnabledWeapons = {"PainKiller","Shotgun","StakeGunGL","MiniGunRL","DriverElectro"}    
    elseif MPCfg.GameMode == "People Can Fly" then    
        ...
        self:AddWeapon(1)
        self.EnabledWeapons = {"PainKiller",nil,"StakeGunGL"}
    end
...
function CPlayer:Steps(delta)
    ...
    if PLAYER.FloorCheck(self._Entity) and INP.Action(2) or INP.Action(4) or INP.Action(8) or INP.Action(16) then 
        local vx,vy,vz,vl = ENTITY.GetVelocity(self._Entity)
        if Vector:New(0,0,0):Dist2D(vx,0,vz) > 7 then
    ...
    -- dzwiek podskoku
    if Game.GMode == GModes.SingleGame and INP.Action(Actions.Jump) and self._JumpSndDelay <= 0 then 
        self._JumpSndDelay = 0.7
    end
...
function CPlayer:SetupAction(clientID,action,pitch,yaw)    
    ...
    PLAYER.AddPlayerAction( action, clientID, pitch, yaw )
...
--============================================================================
function CPlayer:Common_UpdateStats(checkOnly,deadID,killerID,attack_type,gib,frags)
    ...
    if ds then    
        ...
        if checkOnly then
            b_ds = {ds.Score, ds.Frags, ds.Deaths}
            if ks then b_ks = {ks.Score, ks.Frags, ks.Deaths} end
        ...
        if txt and table.getn(txt) > 0 then
            ...
            if not checkOnly then 
                ...
                if ds ~= ks and frags > 1 then
                    if MPCfg.GameMode == "People Can Fly" then 
                        if frags == 2 then CONSOLE.AddMessage(Languages.Texts[697]) end                
                        if frags == 3 then CONSOLE.AddMessage(Languages.Texts[698]) end
                        if frags == 5 then CONSOLE.AddMessage(Languages.Texts[699]) end
                        if frags == 10 then CONSOLE.AddMessage(Languages.Texts[700]) end
                    else
                        CONSOLE.AddMessage(frags.."!!!")
                    end
                end
            end
        ...
        if ks and ks ~= ds then
            if MPCfg.GameMode == "Team Deathmatch" and (attack_type == AttackTypes.TeleFrag or MPCfg.TeamDamage) and ks.Team == ds.Team then
                ks.Score = ks.Score - 1  --  frags
                ks.Frags = ks.Frags + 1  --  kills
            else
                ks.Score = ks.Score + frags  --  frags
                ks.Frags = ks.Frags + 1      --  kills
            end
        else
            ds.Score  = ds.Score  - 1
        ...
        if not checkOnly then 
            MPSTATS.Update(ds.ClientID,ds.Name, ds.Score, ds.Frags, ds.Deaths, ds.Ping, ds.PacketLoss, ds.Team, ds.State, ds.Spectator)
            if ks then
                MPSTATS.Update(ks.ClientID,ks.Name, ks.Score, ks.Frags, ks.Deaths, ks.Ping, ks.PacketLoss, ks.Team, ks.State, ks.Spectator)            
            end
        end
    ...
    for i,o in Game.PlayerStats do
        Game:Print(o.ClientID..", "..o.Name..", "..o.Score..", "..o.Frags..", "..o.Deaths)    
    end
end
--============================================================================
...
function CPlayer:Client_OnDeath(deadID,killerID,attack_type,gib,frags,damage)
    ...
    CPlayer:Common_UpdateStats(false,deadID,killerID,attack_type,gib,frags)
