...
CLevel = 
{
    ...
    s_Editor = {
        ...
        WayPointsMap       = { "BrowseEdit",  {"*.wpm", "data\\maps\\",false} },
...
function CLevel:Apply(old)
    ...
    if not old or (old.Map ~= self.Map or old.Scale ~= self.Scale) then
        if WORLD.GetFileName() == self.Map then -- already loaded
            ...
            WORLD.LoadMap("../Data/Maps/"..self.Map,self._Name,self.Scale,self.Overbright,self.RTCubeMap)        
...
function CLevel:OnCollision(x,y,z,e,h_me,meshName)
	...
	if sound then
		...
		PlaySound3D(snd,x,y,z,sound.dist1,sound.dist2)
	else
		...
		if self._MeshesCollisions then
			...
			if snd then
				...
				PlaySound3D(snds, x,y,z, snd.dist1, snd.dist2)
			else
...
function CLevel:Update()
    ...
    if self._checkRoomTypeDelay < 0 then
        ...
        for i,o in GObjects.Elements do 
            if o._IsAE and o:IsInside(PX,PY+1,PZ) then
                self._r_CurAcEnv = o
                aRoom = o.Room.Type
...
function CLevel:StartAmbientMusic()
    ...
    AddObject(PMusicFade:New(1,SOUND.StreamGetVolume(1),0,5,"SOUND.StreamPause(1);Lev._fade=false"))
    AddObject(PMusicFade:New(0,SOUND.StreamGetVolume(0),Cfg.AmbientVolume,5))
end
--============================================================================
function CLevel:StartBattleMusic()
    ...
    AddObject(PMusicFade:New(1,SOUND.StreamGetVolume(1),Cfg.MusicVolume,2))
    AddObject(PMusicFade:New(0,SOUND.StreamGetVolume(0),0,2,"SOUND.StreamPause(0);Lev._fade=false"))
end
--============================================================================
function CLevel:TryToChangeMusic()
    ...
    if Hud.r_closestEnemy and table.getn(self.Music.Battle) and Cfg.MusicVolume > 0 then 
        self:StartBattleMusic()
