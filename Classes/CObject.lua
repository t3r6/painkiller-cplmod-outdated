...
CObject = 
{
    ...
    _Name = "NoName",
    s_Editor = {
        ["Actions.OnPlay.[new]"]   = { "TextEdit" },
        ["Actions.OnKill.[new]"]   = { "TextEdit" },
        --["Actions.OnLaunch.[new]"] = { "TextEdit" },
    },
}
-- prefixy:
...
--============================================================================
function CObject:Rescale(factor)
    if self.Pos then self.Pos:MulByFloat(factor) end
end
--============================================================================
...
function CObject:TickTimers(delta)
	for i,v in self._Timers do
		...
		while cnt > 0 do
            ...
            cnt = cnt - 1
            --Game:Print("tick")
            if self._ToKill or table.getn(self._Timers) == 0 then break end            
            v[3] = v[3] - v[2]
		end                
	end   
end
...
--============================================================================
function CObject:GetSndInfo(sndID,onlyName)
...
function CObject:Snd2D(id,v)
    local snd,vol,d1,d2,dPitch = self:GetSndInfo(id)
    if v then vol = v end
...
--============================================================================
function CObject:PlaySound(tableR, dist1, dist2, j, noRandomize)
	local name
    local path = nil
	local dist_1 = 12
	local dist_2 = 40
