...
--
	...
	_state = "IDLE",
--	_decalUpdateTimer = 0,
--	_AnimationSounds = {},		-- tablica dzwiekow wywolanych przez animacje, zeby stopowac dzwieki po zmianie animacji
	_angleAttackX = 0,
	_angleAttackY = 0,
	...
	_sndStoppableIDLUA = 0,		-- idLUA, IdC, nameS
	_sndStoppableID_C = nil,
	_sndStoppableNAME = "",
        ...
        for i,o in self._AIBrain._goals do
            if AiStates[o.name] then
                ...
                if self._CustomAiStates and self._CustomAiStates[o.name] then
                    ...
                    MsgBox("Goal not found: "..o.name)
                end
            end            
        end
    end
	if self.s_SubClass and self.s_SubClass.xchgTextures then
		...
		if not self._died then
			...
			if animSpeed > 0 then
				if self.Animation and self._isAnimating and self._AnimationEvents then
					...
					--Game:Print(self.Animation..", curAnimTime"..curAnimTime..", total:"..self._CurAnimLength)
					...
					while self._AnimationEvents[i] do
						...
						if ev[1] <= curAnimTime and ev[2] then
							...
							--	end
							--end
							self[ev[2]](self,ev[3], ev[4], ev[5], ev[6], ev[7])		-- ### pozniej dowolna liczba parametrow
						end
						i = i + 1
					end
	...
	if br and br.r_closestEnemy then
		...
		if dist <= weaponRange and math.abs(aDist) < angleRange*math.pi/180 then
			if damageSound then
				...
				if self.s_SubClass.Sounds and self.s_SubClass.Sounds.damage then
					self:PlayRandomSound2D(self.s_SubClass.Sounds.damage)
				end
			end
...
function CActor:Apply(old)
    if not old or old.Model ~= self.Model or old.Scale ~= self.Scale then 
        ...
        if self.BindRagdoll then
			...
			if b[3] and obj then
				...
				self._procBind = PBindJointToJoint:New(self._Entity, self, b[1], jChild, obj._Entity)
	...
	if self.s_SubClass then
		--[[
		if self.s_SubClass.RagdollCollisions then			-- konwersja nazw jointow na id
			for i,v in self.s_SubClass.RagdollCollisions.Bones do
				if type(v[1]) == "string" then
					local debugError = v[1]
					v[1] = MDL.GetJointIndex(self._Entity, v[1])
					if v[1] == -1 then
						Game:Print(self._Name.." ERROR: kosc w s_SubClass.RagdollCollisions nie istnieje: "..debugError)
					end
				end
			end
		end
		if self.s_SubClass.RagdollCollisionsGib then			-- konwersja nazw jointow na id
			for i,v in self.s_SubClass.RagdollCollisionsGib.Bones do
				if type(v[1]) == "string" then
					local debugError = v[1]
					v[1] = MDL.GetJointIndex(self._Entity, v[1])
					if v[1] == -1 then
						Game:Print(self._Name.." ERROR: kosc w s_SubClass.RagdollCollisionsGib nie istnieje "..debugError)
					end
				end
			end
		end
...
function CActor:SortEvents()
	...
	if self.s_SubClass and self.s_SubClass.Animations then
...
function CActor:SetAnim(anim,loop,animSpeed, blendTime)
    ...
    if self.s_SubClass.Animations and self.s_SubClass.Animations[anim] then
        ...
        self._AnimationEvents = self.s_SubClass.Animations[anim][3]	-- events
	...
	if debugAnim then Game:Print(self._Name.." >Set anim: "..anim.." "..(Game.currentTime/30).." loop: "..Loop..", blend "..blend) end
...
function CActor:Tick(delta)
	if debugMarek then
        ...
        if self._AIBrain then
            Game:Print("V "..self._AIBrain._velocity.." "..isw)
        end--]]
    ...
    --self._CurAnimTime == self._CurAnimLength
		...
		if not self._died then			-- and not self.enabledRD 
			...
			if self._isWalking and not self._enabledRD then
				...
				if --[[self._HasMovingCurve--]] --[[and not self._notIsWalkingTimer or--]] self._moveWithAnimation then
					...
					--if not self._notIsWalkingTimer then
					if not self._speedDamping then
						ENTITY.PO_Move(self._Entity,0,0,0)
					else
						--[[
						x = x * 0.8
						y = y * 0.8
						z = z * 0.8
						ENTITY.PO_Move(self._Entity,,0,0)
						self._speedDamping--]]
					end
					--	Game:Print("not is walking to move 0,0,0")
					--else
					--	Game:Print("not is walking to move ???")
					--end
					if debugMarek then
						self._debugPOMOVEy = 0
                        self._debugPOMOVEx = 0
                        self._debugPOMOVEz = 0
					end
				end
...
function CActor:MoveWithAnimation(delta)
	local mvx,mvy,mvz
	local x,y,z = self:GetAnimMovement(delta)
...
function CActor:UpdateWalking(delta)
	...
	if self._walkWithAngle and not self._isRotating then
	...
	if self._AIBrain and self.AIenabled then
		...
		if self._lastCheck and self._lastCheck ~= br._currentTime then
			...
			if self._lastShouldMove then
				if self._butMove < self._shouldMove * self.CollisionDetect and self._butMove < self._lastbutMove and self._shouldMove > 0.01 then
	...
	if gib then
		...
		if not self._disableGibSound then
            ...
            if self._frozen then 
                s = {"impacts/gib_frozen1","impacts/gib_frozen2","impacts/gib_frozen3"}
            else
                s = {"impacts/gib_big","impacts/gib_big2","impacts/gib_big3"}
            end
			local snd = s[math.random(1,3)]
			PlaySound3D(snd, x,y,z, 16, 50)
	    ...
	    if self.s_SubClass and self.s_SubClass.CollidableRagdoll then 
...
function CActor:StdRagdollOnCollision(x,y,z,nx,ny,nz,e_other,h_me,h_other,vx,vy,vz,vl, velocity_me, velocity_other)	--e.he
	...
	if j and j ~= -1 then
		if self._raggDollPrecomputedCollData[j] then
            ...
            local sndDir = ""
            if self._frozen then
				name = "impacts/meat_frozen-splash"..math.random(1,4)
			else
				local s = self._raggDollPrecomputedCollData[j][1]
                if type(s) == "table" then
					name = s[math.random(1, table.getn(s))]
				end


                if self.s_SubClass.SoundsDefinitions then
                    local sndDef = self.s_SubClass.SoundsDefinitions[s]
                    if sndDef then
                        name = sndDef.samples
                    end
                else
			...
			local mul = 1.0
			if self.s_SubClass.deathSoundRangeMult then
				mul = self.s_SubClass.deathSoundRangeMult
			end
            PlaySound3D(sndDir..name, x,y,z, 16*mul, 60*mul)
...
function CActor:FlyForward(dist, ang, up)
	...
	v:Normalize()
	return self:FlyTo(xd + v.X*dist, yd, zd + v.Z*dist)
end	

function CActor:FlyTo(destx,desty,destz, Run, animName)
	local x,y,z = self._groundx,self._groundy,self._groundz
	...
	self:SetAnim(anm, true, animSpeed)
...
function CActor:WalkTo(destx,desty,destz, Run, maxDist, animName, useOnlyWP, walkWithAngle, moveBackward)
	...
	self._notIsWalkingTimer = nil
	--Game:Print(self._Name..": WalkTo: "..d1)
	if not self.doNotUseWP then
		PATH.GetShortest(self._Path,x,y,z,destx,desty,destz)
	end
    self._destx = destx
    self._desty = desty
    self._destz = destz
...
function CActor:SetIdle(once)
	...
	if not self.s_SubClass.Animations.idle or not self:SetAnim("idle", loop) then
		if self.s_SubClass and self.s_SubClass.Ambients then
			...
			if self.s_SubClass.Animations[animName] then
				return self:SetAnim(animName, false)
			end
		end
		return false
	else
...
--============================================================================
function CActor:BindPoint(ox,oy,oz)
    local a = -self.angle
...
--============================================================================
function CActor:OnDamage(damage, obj, type, x, y, z, nx, ny, nz, he)			-- +pos
	if not self._died then
		if Game.Cheat_WeakEnemies then self.Health = 1 end
		...
		if self.Health <= 0 then
			...
			if not self._ToKill then
				...
				if self.s_SubClass.Sounds and self.s_SubClass.Sounds.death and not self._disableDeathSounds then
                    local s = self.s_SubClass.Sounds.death
                    local snd = s[math.random(1,table.getn(s))]
                    local deathSoundRangeMult = 1.0
                    if self.s_SubClass.deathSoundRangeMult then
						deathSoundRangeMult = self.s_SubClass.deathSoundRangeMult
                    end
					self:BindSound(self._SoundDirectory..snd,30 * deathSoundRangeMult,70 * deathSoundRangeMult, false, self:GetAnyJoint())
				end
			end
			...
			--end
			if Tweak.MonsterCanAttackAnotherMonsterChance and not self.disableFreeze then
				if debugMarek then
					if obj and not obj.Health then
						Game.freezeUpdate = true
						Game:Print(obj._Name.." OBIEKT BEZ HEALT")
					end
					if not self._AIBrain then
						Game.freezeUpdate = true
						Game:Print(self._Name.." OBIEKT BEZ AIBRAIN")
					end
				end
			...
			if not cantDo then
				if self.s_SubClass.Sounds and self.s_SubClass.Sounds.hurt then
					...
					--end
					self:PlaySoundHit(self.s_SubClass.Sounds.hurt)
				end
...
function CActor:ExplodeBody()
    ...
    if tdj then
        ...
        if self.AiParams.madonnaFX then
			...
			if self.s_SubClass.Sounds and self.s_SubClass.Sounds.OnDisapear then
				self:PlayRandomSound3D(self.s_SubClass.Sounds.OnDisapear)
			end
		else
...
function CActor:Trace(length, ang)
    local cx,cy,cz = self._groundx,self._groundy + self._SphereSize, self._groundz
   	local angle = self.angle
   	if ang then
    ...
    local b,d,x,y,z,nx,ny,nz,he,e = WORLD.LineTrace(cx,cy,cz,fx,fy,fz)	-- FixedGeom
    ENTITY.AddRagdollToIntersectionSolver(self._Entity)
	...
	if self.s_SubClass and self.s_SubClass.ParticlesDefinitions then
		...
		if fx then
			...
			if rotateWithJoint then
				PARTICLE.SetParentOffset(pfx,fx.offset.X,fx.offset.Y,fx.offset.Z,joint,nil,nil,nil,0,0,math.pi/2)
			else
...
--end
--============================================================================
function CActor:UnregisterAllChildren()
...
--    end
    ...
    if not self._died then        
        self.Immortal = nil
        self.Health = 1
        self:OnDamage(99999,self,AttackTypes.OutOfLevel)
        GObjects:ToKill(self)
    else
...
function CActor:PlayRandomSound2D(tableR, noRandomize)
	local name = tableR[math.random(1, table.getn(tableR))]
	if name ~= "" then
		local res = string.find(name,"$/")
		local sndDir
		if res then
...
function CActor:BindRandomSound(tableR, dist1, dist2, bjoint, doNotAutodelete, loop, disablePitch)
	...
	if name ~= "" then
		local snd, idx = self:BindSound(self._SoundDirectory..name, dist_1, dist_2, loop, joint,nil,nil,nil,doNotAutodelete, disablePitch)
		return snd, idx
	end
end
...
function CActor:PlaySoundAndStopLast(name, dist1, dist2, noRandomize)		-- nastepny grany dzwiek kasuje poprzedni (z fade out-tem)
    ...
    if type(name) == "table" then
		...
		-- z tablicy definicji
		if self.s_SubClass.SoundsDefinitions then
			local sndDef = self.s_SubClass.SoundsDefinitions[name]
			if sndDef then
				if sndDef.playChance then
					if FRand(0,1) > sndDef.playChance then
						return
	...
	local snd = self._sndStoppableID_C
	if snd then
		if SOUND3D.IsPlaying(snd) then
			SOUND3D.SetVolume(snd, 0, 0.2)
			SOUND3D.Forget(snd)
		else
			SOUND3D.Delete(snd)
		end		
		self._sndStoppableID_C = nil
		self._sndStoppableNAME = ""
	end
	
	local id = SOUND3D.Create(self._SoundDirectory..name)
	local x,y,z = self._groundx,self._groundy,self._groundz
	if joint then
		x,y,z = self:GetJointPos(joint)
	end
	SOUND3D.SetPosition(id, x,y,z)
	 SOUND3D.Forget(id)
	SOUND3D.SetHearingDistance(id, dist_1, dist_2)

	SOUND3D.Play(id)
	if disablePitch then
		SOUND3D.DisableRandomize(id)
	end
	
	self._sndStoppableNAME = name
	self._sndStoppableID_C = id
	self._sndStoppableIDlua = self._sndStoppableIDLUA + 1
	return self._sndStoppableIDLUA			-- zwraca idLUA
end

function CActor:PlaySoundHit(par3, par4, par5)			-- tylko dla dzwiekow "hit", zeby nie odgrywal sie nowy, gdy jest stary
	local name = par3
	if type(par3) == "table" then
		name = par3[math.random(1, table.getn(par3))]
	end
	if name ~= "" then
		if self._sndHitId then
			if self:IsPlayingLastSound(self._sndHitId) then
				return
			end
		end
...
function CActor:PlaySoundHitIfNotPlayingLast(par3, par4, par5)		-- gra dziwk, jesli inny nie jest grany, i nie jest pamietany, ze gra
	local name = par3
	if type(par3) == "table" then
		name = par3[math.random(1, table.getn(par3))]
	end
	...
	return self:PlaySoundAndStopLast(name, par4, par5)
end


function CActor:IsPlayingLastSound(idLUA)
	local snd = self._sndStoppableID_C
	if snd and self._sndStoppableIDLUA == idLUA then
		if SOUND3D.IsPlaying(snd) then
			return true
		end
	end
...
function CActor:StopLastSound(idLUA)
	...
	if snd then
		if idLUA then
			if self._sndStoppableIDLUA == idLUA then
				if SOUND3D.IsPlaying(snd) then
					SOUND3D.SetVolume(snd, 0, 0.2)
					SOUND3D.Forget(snd)
					--Game:Print("SND: "..self._sndStoppableNAME.." forget on demand")
				else
					SOUND3D.Delete(snd)
					--Game:Print("SND: "..self._sndStoppableNAME.." delete on demand")
				end		
				self._sndStoppableID_C = nil
				self._sndStoppableNAME = ""
				return true
			...
			--Game:Print("stop last sound:")
			if SOUND3D.IsPlaying(snd) then
				SOUND3D.SetVolume(snd, 0, 0.1)
				SOUND3D.Forget(snd)
				--Game:Print("SND: "..self._sndStoppableNAME.." forget on demand any")
			else
				SOUND3D.Delete(snd)
				--Game:Print("SND: "..self._sndStoppableNAME.." delete on demand any")
			end		
			self._sndStoppableID_C = nil
			self._sndStoppableNAME = ""
			return true
...
---------------------------
function CActor:PlaySoundHitBinded(par3, par4, par5, joint, loop)			-- tylko dla dzwiekow "hit", zeby nie odgrywal sie nowy, gdy jest stary
	local snd
	local name = par3[math.random(1, table.getn(par3))]
	if name ~= "" then
		if self._sndStoppableID_CBinded and SND.IsPlaying(self._sndStoppableID_CBinded) then
			...
			--if debugMarek then Game:Print("Binded still playing. Stopped last. New: "..name) end
			
			--Game:Print("Binded still playing. Canceled: "..name)
			--ENTITY.Release(self._sndStoppableID_CBinded)
			--self._sndStoppableID_CBinded = nil
			--return
		end
		--Game:Print("Binded play: "..name)
		snd, self._sndStoppableID_CBinded = self:BindRandomSound(name, par4, par5, joint, nil, loop)
	end
	return snd
end
----------------------------------
...
CActor.PlayRandomSound3D = CActor.PlaySound				-- pozniej to uporzadkowac


function CActor:FootFX(joint)
...
function CActor:ThrowImmid(angle, straight, joint, angleDiff)		-- bool straight/ballistic
    local aiParams = self.AiParams
    local j = joint
    if not j then
...
function CActor:ThrowTaken(forceYaw, straight, angleDiff)		-- bool straight(true)/ballistic(false)
    ...
    if self._proc then
		...
		if self._objTakenToThrow then
			x1,y1,z1 = self._objTakenToThrow.Pos.X,self._objTakenToThrow.Pos.Y,self._objTakenToThrow.Pos.Z
		end
    end
...
function CActor:UpdateRotateWithAnim(delta)
	--Game:Print("UpdateRotateWithAnim() "..self._angleDestAnim.. " "..self.Animation)
...
function CalcThrowVectorGivenAngle(distance, angle, angleXZ, deltaY)		-- given angle
    ...
    if temp > 0 then
        ...
        Game:Print("ERROR: force < 0 actor cant throw at desired target, with that angle")
    end
