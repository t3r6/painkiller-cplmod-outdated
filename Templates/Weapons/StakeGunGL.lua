...
--============================================================================
function StakeGunGL:Fire(prevstate,combo) -- Stake
    ...
    if self.ObjOwner.Ammo.Stakes > 0 then       
        ...
        self.FireSFX(self.ObjOwner._Entity,obj._Entity,combo)
        return obj
    else
...
--============================================================================
function StakeGunGL:Combo()       
    if self.ObjOwner.Ammo.Grenades > 0  then                     
        Game:Print("combo")
        local stake = self:Fire(nil,1)
        if stake then
            stake.combo = true
            local fv = self.ObjOwner.ForwardVector
            local vx,vy,vz = ENTITY.GetVelocity(stake._Entity)
            ENTITY.SetVelocity(stake._Entity,vx/2,vy/2,vz/2)        
        end
    else
        self:Fire()        
    end
...
-- NET EVENTS
--============================================================================
function StakeGunGL:FireSFX(pe,se,combo)
    ...
    if player then
        ...
        if combo == 1 then player.Ammo.Grenades = player.Ammo.Grenades - 1 end
...
Network:RegisterMethod("StakeGunGL.FireSFX", NCallOn.ServerAndAllClients, NMode.Reliable, "eeb") 
