...
function o._CustomAiStates.DemonIdle:OnUpdate(brain)
	...
	--Game:Print("_velocity "..brain._velocity)

	if brain._seeEenmy then
		brain._distToNearestEnemy = Dist3D(brain.r_closestEnemy._groundx,brain.r_closestEnemy._groundy + 2.0,brain.r_closestEnemy._groundz, actor._groundx,actor._groundy,actor._groundz)
	end
	...
	if not actor._isWalking and not actor._isRotating then
		...
		if (math.random(100) < 30 and not self._retryWalk) or (self._retryWalk and math.random(100) < 10) then
			...
			if math.random(100) < 50 and not self._retryWalk then
				self._wait = FRand(10,16)
			else
			
				local x,y,z
				
				local dist = FRand(4,11)
				local up = FRand(-1.4,1.4)
				local ang = nil
				...
				if brain.r_closestEnemy then
					...
					Game:Print("_distToNearestEnemy = "..brain._distToNearestEnemy)
					-- atak na playera
					...
					local chance = 0.8
					if brain._distToNearestEnemy >= 14 then		-- albo przeszkoda (disttoplayer spr)
						chance = 1 - (brain._distToNearestEnemy - 14)/(actor.AiParams.viewDistance-14)
						Game:Print(brain._distToNearestEnemy.." chance "..chance)
						chance = chance * 0.3
					end
					
					if --[[brain._distToNearestEnemy < 15 and]] FRand(0,1) < chance then		-- albo przeszkoda (disttoplayer spr)
						...
						dist = brain._distToNearestEnemy * FRand(0.3, 0.5)
						if dist < 2 then
							dist = 2
						end
						
						ang = FRand(-43,43)
						up = (brain._enemyLastSeenPoint.Y + 2.0) - actor._groundy + FRand(0,0.8)
					end
					-- ktory atak?
					--
				end
...
function o._CustomAiStates.DemonAtak:OnUpdate(brain)
	...
	--if not actor._isWalking then
		brain._atak = false
		actor:FlyForward(FRand(2.6,5),FRand(-50,50), FRand(-0.5, 1.5))
