...
----------------------------
AiStates.ninjaTeleport = {
	name = "ninjaTeleport",
	_lastTimeTeleport = 0,
}

function AiStates.ninjaTeleport:OnInit(brain)
    ...
    if enemy then
		...
		if maxCount >= 0 then
			...
			actor:PlayRandomSound3D({"ninja_sprint"})
			actor:PlayRandomSound3D({"ninja_at2-shurican","ninja_at3-shurican"})
...
function AiStates.ninjaTeleport:OnUpdate(brain)
    local actor = brain._Objactor
	local aiParams = actor.AiParams
	self._lastTimeTeleport = brain._currentTime
...
function AiStates.ninjaTeleport:OnRelease(brain)
    local actor = brain._Objactor
...
function AiStates.ninjaTeleport:Evaluate(brain)
	if self.active then
		return 0.8
	end
	if brain.r_closestEnemy then
