...
function Skull:Catch()
	...
	if h and h.Health > 0 then
		...
		self._proc = PBindJointToJoint:New(self._Entity, self, aiParams.holdJoint, brain._JointH, h._Entity)
		GObjects:Add(TempObjName(), self._proc)

		if h.s_SubClass.Sounds.catch then
			h:PlaySound(h.s_SubClass.Sounds.catch,12,26,"root")
		end
...
function AiStates.TakeHostage:OnInit(brain)
...
function AiStates.TakeHostage:OnUpdate(brain)
	local actor = brain._Objactor
    local hostage = brain.Objhostage
    local aiParams = actor.AiParams
    if hostage then
...
function AiStates.TakeHostage:OnRelease(brain)
	local actor = brain._Objactor
	self.active = false
...
function AiStates.TakeHostage:Evaluate(brain)
	local actor = brain._Objactor
	local aiParams = actor.AiParams
