...
function Giant._CustomAiStates.idleGiant:OnUpdate(brain)
	...
	if self.lastTimeAmbientSound + 5 < brain._currentTime then	
		if math.random(1000) < 4 and actor.s_SubClass.Sounds.ambients then
			self.lastTimeAmbientSound = brain._currentTime
			local name = actor.s_SubClass.Sounds.ambients[math.random(1, table.getn(actor.s_SubClass.Sounds.ambients))]
			PlaySound3D("actor/"..actor.Model.."/"..name,actor._groundx,actor._groundy,actor._groundz,10,25,self)
			--Game:Print("PLAY AMBIENT SOUND")
		end 
	end
