...
AiStates.amputeeIdle = {
	name = "amputeeIdle",
	_Class = "CAiGoal",

}

function AiStates.amputeeIdle:OnInit(brain)
...
function AiStates.amputeeIdle:OnUpdate(brain)
	local actor = brain._Objactor
...
function AiStates.amputeeIdle:OnRelease(brain)
	self.active = false
end

function AiStates.amputeeIdle:Evaluate(brain)
	return 0.1
end


------------
AiStates.amputeeAttack = {
	name = "amputeeAttack",
	_Class = "CAiGoal",
...
function AiStates.amputeeAttack:OnInit(brain)
	local actor = brain._Objactor
	self.active = true
...
function AiStates.amputeeAttack:OnUpdate(brain)
	local actor = brain._Objactor
	local aiParams = actor.AiParams
...
function AiStates.amputeeAttack:OnRelease(brain)
	self.active = false
	local actor = brain._Objactor
	if actor._proc then
		GObjects:ToKill(actor._proc)
...
function AiStates.amputeeAttack:Evaluate(brain)
	if self.active then
		return 0.2
	end
	...
	if math.random(100) < 5 then
		...
		if not self._oneTime and brain.r_closestEnemy then
			actor:PlayRandomSound3D(actor.s_SubClass.Sounds.onAttackOnce)
			self._oneTime = true
		end
