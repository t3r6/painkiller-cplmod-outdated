...
function Banshee:Attack()
	...
	if br and br.r_closestEnemy then
		...
		if dist <= aiParams.attackRange and math.abs(aDist) < aiParams.attackRangeAngle*math.pi/180 then
			...
			PlaySound2D(self._SoundDirectory..self.s_SubClass.Sounds.damage)
...
function Banshee:Scream()
	local aiParams = self.AiParams
	self._sndScream = self:PlaySoundAndStopLast(self.s_SubClass.Sounds.scream,aiParams.screamDistance,aiParams.screamDistanceMax)
end
...
function AiStates.bansheeAttack:OnInit(brain)
	local actor = brain._Objactor
	self.mode = 0
end

function AiStates.bansheeAttack:OnUpdate(brain)
	local actor = brain._Objactor
	local aiParams = actor.AiParams
	
	if actor.s_SubClass.Sounds.attackAmbient then
		if self.lastTimeOnAttackAmbient + aiParams.soundAmbientDelay < brain._currentTime and math.random(100) < 3 then
			actor._lastAttackAmbient = actor:PlaySoundHitIfNotPlayingLast(actor.s_SubClass.Sounds.attackAmbient)
...
function AiStates.bansheeAttack:OnRelease(brain)
	local actor = brain._Objactor
	--Game:Print("BANSHEE release")
end


function AiStates.bansheeAttack:Evaluate(brain)
