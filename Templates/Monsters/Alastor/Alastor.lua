...
function Alastor:CustomOnDamage(he,x,y,z,obj,damage,type,nx,ny,nz)
	...
	if not self._ABdo then
		...
		if self._AIBrain._lastDamageTime + 1.5 < self._AIBrain._currentTime then
			self:PlaySoundHitBinded(self.s_SubClass.Sounds.hurt, 50, 200)
	...
	if actor._phase == 0 then
		...
		if self.mode == 1 then
			if self.delay then
				...
				AddObject(PMusicFade:New(1,SOUND.StreamGetVolume(1),Cfg.MusicVolume,0.1))
				AddObject(PMusicFade:New(0,SOUND.StreamGetVolume(0),0,0.1,"SOUND.StreamPause(0);Lev._fade=false"))
...
function Alastor:CreateMonks()
    ...
    if monks then
        ...
        for i,v in monks.Points do
            local r = Quaternion:New_FromEulerZYX(0,-v.A+math.pi/2,0)
